...
class BasePlanner(CustomPlanner):
    ...
    def train_vgp(
        ...
        if cross_validate and train_y.shape[0] >= min_obs:
            idx_0 = torch.where(train_y == 0.0)[0]
            idx_1 = torch.where(train_y == 1.0)[0]
            ...
            indices = torch.randperm(
                train_y_infl.shape[0], dtype=torch.long
            )  # torch.arange(train_y_infl.shape[0])

            # cross validation procedure
            for fold_ix in range(num_folds):
                # create fold data
                train_x_fold, train_y_fold = (
                    train_x_infl[indices[fold_size:], :],
                    train_y_infl[indices[fold_size:]],
                )
                ...
                folds.append(
                    {
                        "model": model_fold,
                        "likelihood": likelihood_fold,
                        "optimizer": optimizer_fold,
                        "mll": mll_fold,
                        "train_x": train_x_fold,
                        "train_y": train_y_fold,
                        "valid_x": valid_x_fold,
                        "valid_y": valid_y_fold,
                    }
                )
            ...
            for fold_ix, fold in enumerate(folds):
                ...
                with gpytorch.settings.cholesky_jitter(self.max_jitter):
                    ...
                    patience_iter_ = 0
                    for iter_ in track(
                        range(self.vgp_iters),
                        description=f"Training variational GP on fold {fold_ix+1}/{num_folds}",
                    ):
            ...
            with gpytorch.settings.cholesky_jitter(self.max_jitter):
                for iter_ in track(
                    range(num_iters_full),
                    description=f"Training variational GP on all observations ...",
                ):
                    optimizer.zero_grad()
    ...
    def fca_constraint(self, X: torch.Tensor) -> torch.Tensor:
        ...
        with gpytorch.settings.cholesky_jitter(1e-1):
            ...
            constraint_val = (1.0 - p_infeas) - self.fca_cutoff
    ...
    def initial_design(self) -> List[ParameterVector]:
        ...
        if num_init_remain > 0:
            if num_init_remain % self.batch_size == 0:
                ...
                Logger.log(
                    f"Generating {num_gen} initial design points (batch {init_batch_num}/{num_init_batches})",
                    "INFO",
                )
            else:
            ...
            num_gen = self.batch_size
            Logger.log(
                f"Found all NaN observations after initial design. Generating {num_gen} additional points",
                "WARNING",
            )
        else:
        ...
        while len(return_params) < num_gen:
            ...
            if self.init_design_strategy == "random":
                self.init_design_planner._tell(
                    iteration=self.num_init_design_attempted
                )
            else:
        ...
        if self.known_constraints.has_compositional_constraint:
            ...
            for pvec in return_params:
                ...
                constrained_pvec[
                    self.known_constraints.compositional_constraint_param_names[
                        -1
                    ]
                ] = (1.0 - sum_params)
        ...
        if self.known_constraints.has_batch_constraint:
            ...
            for pvec in return_params:
                ...
                for (
                    ...
                    constrained_pvec[constrained_param_name] = return_params[
                        0
                    ][constrained_param_name]
