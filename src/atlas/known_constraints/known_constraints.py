...
class CompositionalConstraint:
    ...
    def __init__(
        self, compositional_params: List[int], param_space: ParameterSpace
    ) -> None:
    ...
    def __call__(self, params) -> bool:
        ...
        if (
            np.sum(
                [float(params[idx]) for idx in self.compositional_params[:-1]]
            )
            > 1.0
        ):
            return False
    ...
    def _validate(self):
        ...
        if not all(
            [
                self.param_space[idx].type in ["continuous", "discrete"]
                for idx in self.compositional_params
            ]
        ):
            Logger.log(
...
class PermutationConstraint:
    ...
    def __init__(
        self, permutation_params: List[int], param_space: ParameterSpace
    ) -> None:
    ...
    def _validate(self):
        ...
        if not np.logical_or(
            ...
            all(
                [
                    self.param_space[idx].type == "categorical"
                    for idx in self.permutation_params
                ]
            ),
        ):
...
class PendingExperimentConstraint:
    ...
    def __init__(
        self,
        pending_experiments: List[ParameterVector],
...
class KnownConstraints:
    ...
    def batch_constrained_param_names(self):
        return [
            self.param_space[idx].name for idx in self.batch_constrained_params
        ]

    @property
    def compositional_constraint_param_names(self):
        return [
            self.param_space[idx].name for idx in self.compositional_params
        ]
